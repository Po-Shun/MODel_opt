...
class Scheduler:
    ...
    def ComputeOptimalSchedule(
        ...
        if not allow_rematerialization and not allow_swaps and not mix_approach:
            num_timesteps = min(
        ...
        for e in self.graph.edges.values():
            ...
            if e.source in schedule_constraints:
                ...
                assert ts <= ub
                for t in self.TimeStepsForEdge(e, spans):
                    if t != ts and not allow_rematerialization and not mix_approach:
        ...
        for e, ts in generate_vars.items():
            ...
            for v in ts.values():
                s += v
            if (
                (mix_approach or allow_rematerialization)
        ...
        if mix_approach:
            ...
            if not min_memory_requirement_acurate and not spilling_allowed:
                ...
                for ts, mem_usage in mem_at_timestep.items():
                    solver.add_constraint(
                        mem_usage <= max_usage,
                        name=f"bound_peak_mem_usage{ts}"
                    )
        ...
        if max_spills is not None and not allow_rematerialization and not mix_approach:
        ...
        if allow_rematerialization or mix_approach:
    ...
    def ComputeOptimalSchedule_V2(
        ...
        print(f"[DEBUG] mix_approach:{mix_approach}, alpha:{alpha}, beta:{beta}")
        # Compute the minimum amount of memory required to run the graph.
        min_memory_requirement, bottleneck_node = self.ComputeMinimumMemoryRequired()
        min_memory_requirement_acurate = False
        if min_memory_requirement > mem_limit:
